...
- (void)viewDidLoad
{
    ...
    //tableView列表显示
    _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, STATUS_HEIGHT + NAVIGATION_HEIGHT, DEVICE_WIDTH, DEVICE_HEIGHT - STATUS_HEIGHT - NAVIGATION_HEIGHT-49) style:UITableViewStylePlain];
    ...
    //给定固定cell名和图标，存在数组中
    cell_name_array = [[NSArray alloc]initWithObjects:@"添加新设备",@"导出数据",@"导入数据",@"按键震动",@"APP下载链接",@"FAQ",@"用户手册",@"关于晟大紫光", nil];
    cell_image_array = [[NSArray alloc]initWithObjects:[UIImage imageNamed:@"icon_add_device"],[UIImage imageNamed:@"icon_upload_data"],[UIImage imageNamed:@"icon_download_data"],[UIImage imageNamed:@"icon_shake"],[UIImage imageNamed:@"icon_qrcode"],[UIImage imageNamed:@"icon_faq"],[UIImage imageNamed:@"icon_faq"],[UIImage imageNamed:@"icon_about"], nil];
...
//导出数据,点击弹出提示框
    if (row == 1)
    {
        UIAlertView * export_data = [[UIAlertView alloc]initWithTitle:@"导出数据" message:@"数据已经准备导出，请需要数据的手机按“导入数据”按钮，进行连接！点击“取消”后，停止共享数据..." delegate:self cancelButtonTitle:@"取消" otherButtonTitles: nil];
...
//导入文件
    if (row ==2)
    {
        ...
        //TODO:表格的添加
        UIActionSheet * import_data = [[UIActionSheet alloc]initWithTitle:@"搜索到的设备" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles: nil];
        UIButton * btn = [UIButton buttonWithType:UIButtonTypeCustom];
        [btn setTitle:@"device_name" forState:UIControlStateNormal];
        //        tableView_import = [[UITableView alloc]initWithFrame:CGRectMake(10, 35, import_data.frame.size.width-20, import_data.frame.size.height-49-20) style:UITableViewStylePlain];
        ...
        device.frame = CGRectMake(10, 35, import_data.frame.size.width-20, 45);
        [device addTarget:self action:@selector(broadAddress) forControlEvents:UIControlEventTouchUpInside];
...
{
    NSError * error = nil;
    _udpSocket = [[GCDAsyncUdpSocket alloc]initWithDelegate:self delegateQueue:GLOBAL_QUEUE socketQueue:socketQueue];
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didReceiveData:(NSData *)data fromAddress:(NSData *)address withFilterContext:(id)filterContext
{
    ...
    [GCDAsyncUdpSocket getHost:&address1
                          port:&port
                   fromAddress:address];
...
- (void)startToExport
{
    ...
    if (![self.tcpSocket acceptOnPort:BIND_PORT
                                error:&error]) {
