...
//    NSLog(@"serverIp=%s\n",ipadd);
    ...
    Byte *bytes = [CC3xMessageUtil ip2HexBytes:[NSString stringWithCString:ipadd encoding:NSUTF8StringEncoding]];
...
// P2S_LOCATE_REQ 0x3b
+ (NSData *)getP2SMsg3B:(NSString *)mac
                     on:(BOOL)on
{
    p2sMsg3B msg;
    memset(&msg, 0, sizeof(msg));
...
+ (NSData *)getP2SMsg59:(NSString *)mac
{
    ...
    const char *systemName=[[[UIDevice currentDevice] systemVersion] UTF8String];
    if (sizeof(name)>20){
...
+ (CC3xMessage *)parseD2P02:(NSData *)aData
{
    ...
    [NSString stringWithFormat:@"%02x:%02x:%02x:%02x:%02x:%02x",
     msg.mac[0], msg.mac[1],msg.mac[2],msg.mac[3],msg.mac[4],
     msg.mac[5]];
    
    message.ip = [NSString stringWithFormat:@"%d.%d.%d.%d",
...
+ (CC3xMessage *)parseD2P0C:(NSData *)aData
{
    ...
    message.deviceName =
    [[NSString alloc] initWithBytes:msg.deviceName
                             length:sizeof(msg.deviceName)
                           encoding:NSUTF8StringEncoding];
    message.version = msg.FWVersion;
...
+ (CC3xMessage *)parseS2P36:(NSData *)aData
{
    ...
    message.msgDir = msg->header.msgDir;
    message.msgLength = msg->header.msgLength;
    message.mac = [NSString stringWithFormat:@"%02x-%02x-%02x-%02x-%02x-%02x",msg->mac[0],msg->mac[1],msg->mac[2],msg->mac[3],msg->mac[4],msg->mac[5]];
    message.state = msg->state;
    message.power = msg->power;
    return message;
}
...
+ (CC3xMessage *)parseD2P54:(NSData *)aData
{
    ...
    message.msgLength = msg.header.msgLength;
    message.mac = [NSString stringWithFormat:@"%02x-%02x-%02x-%02x-%02x-%02x",msg.mac[0],msg.mac[1],msg.mac[2],msg.mac[3],msg.mac[4],msg.mac[5]];
    message.delay = msg.delay/256;//这个地方不知道什么原因导致左移两位，放大了256倍
    message.isOn = msg.on;
    message.crc = msg.crc;
    return message;
}
...
+ (CC3xMessage *)parseS2P6A:(NSData *)aData
{
    ...
    message.updateUrl =[NSString stringWithCString:msg->updateUrl encoding:NSUTF8StringEncoding] ;
    return message;
}
...
//                                       message:NSLocalizedString(@"Message error, please try again", nil)
//                                      delegate:nil
...
+ (NSData *)data2HexData:(NSData *)data
{
	...
	NSAssert(0 == length % 2, @"Hex strings should have an even number of digits");
	...
	for (CFIndex i = 0; i < length; i += 2) {
		...
		char *b2 = NULL;
		*bp++ = strtol(buf, &b2, 16);
		NSAssert(b2 == buf + 2, @"String should be all hex digits: (bad digit around %ld)", i);
	}
