...
- (void)viewDidLoad
{
    ...
    [left addTarget:self action:@selector(backToDevicesWithTimer) forControlEvents:UIControlEventTouchUpInside];
    ...
    [right addTarget:self action:@selector(refreshTimerList) forControlEvents:UIControlEventTouchUpInside];
...
//    [label setBounds:CGRectMake(100 , 20, 60, 20)];
    ...
    timer_table = [[UITableView alloc]initWithFrame:CGRectMake(25, 40, DEVICE_WIDTH-50, DEVICE_HEIGHT-127)style:UITableViewStylePlain];
//    [timer_table setBounds:CGRectMake(0, 45, DEVICE_WIDTH-60, DEVICE_HEIGHT-127)];
    ...
    [_addTimer_btn setImage:[UIImage imageNamed:@"smartplug_timer_add_button"] forState:UIControlStateNormal];
    [_addTimer_btn setFrame:CGRectMake(265, 4, 30, 30)];
    [_addTimer_btn addTarget:self action:@selector(addTimer) forControlEvents:UIControlEventTouchUpInside];
   ...
   didHandleLongPress:(UIGestureRecognizer *)recognizer
{
    ...
    [[UIActionSheet alloc] initWithTitle:nil
                  ...
                  destructiveButtonTitle:nil
                       otherButtonTitles:NSLocalizedString(@"delete", nil),NSLocalizedString(@"Edit", nil), nil];
...
- (void)setDayOfTimer:(TimerTableViewCell *)cell
                 task:(CC3xTimerTask *)aTask{
    //显示选中的重复weekday
    ...
    if ([aTask isDayOn:THURSDAY]) {
        [cell.thursdayButton setBackgroundImage:image forState:UIControlStateNormal];
    } else {
        [cell.thursdayButton setBackgroundImage:image1 forState:UIControlStateNormal];
    }
    ...
    if ([aTask isDayOn:SATURDAY]) {
        [cell.saturdayButton setBackgroundImage:image forState:UIControlStateNormal];
    } else {
        [cell.saturdayButton setBackgroundImage:image1 forState:UIControlStateNormal];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    cell = (TimerTableViewCell *)[tableView dequeueReusableCellWithIdentifier:cellId];
    if (cell == nil){
        cell = [[TimerTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
    NSUInteger row = indexPath.row;
    //如果定时列表为空，直接返回
    CC3xTimerTask *task = self.timerList[row];
    cell.open_field.text = [NSString stringWithFormat:@"%02u:%02u",task.startTime / 3600, (task.startTime % 3600) / 60];
    cell.open_status_label.text = task.isStartTimeOn ? NSLocalizedString(@"Run", nil) : NSLocalizedString(@"Not run", nil);
    cell.close_field.text = [NSString stringWithFormat:@"%02u:%02u",task.endTime / 3600, (task.endTime % 3600) / 60];
    cell.close_status_label.text = task.isEndTimeOn ? NSLocalizedString(@"Run", nil) : NSLocalizedString(@"Not run", nil);
    cell.set_switch.on = task.isTaskOn ? YES : NO;
    [cell.set_switch addTarget:self action:@selector(toggleSwitch:) forControlEvents:UIControlEventValueChanged];
...
{
    [timer_table deselectRowAtIndexPath:indexPath
                                  animated:YES];
...
- (void)toggleSwitch:(id)sender
{
    ...
    CGPoint cellPoint = [us.superview convertPoint:center
                                            toView:timer_table];
    NSIndexPath *indexPath =
    [timer_table indexPathForRowAtPoint:cellPoint];
    self.selectedCell = (TimerTableViewCell *)[timer_table cellForRowAtIndexPath:indexPath];
...
{
//    if (self.delegate && [self.delegate respondsToSelector:@selector(timer:)])
//    {
