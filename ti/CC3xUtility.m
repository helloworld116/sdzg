...
+ (void)setupUdpSocket:(GCDAsyncUdpSocket *)socket
                  port:(uint16_t)aPort
{
   NSError *error = nil;
...
{
   dispatch_queue_t filterQueue = dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT,0);
   ...
   [socket setReceiveFilter:filter
                  withQueue:filterQueue];
}
...
+ (void)networkNotReachableAlert
{
   ...
   [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Warning", nil)
                              message:NSLocalizedString(@"WiFi not available. Please check your WiFi connection", nil)
                             delegate:nil
