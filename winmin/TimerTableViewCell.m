...
@synthesize open_field, open_label, open_status_label, close_field, close_label,
    close_status_label, set_switch, mondayButton, tuesdayButton, wensdayButton,
    thursdayButton, fridayButton, saturdayButton, sundayButton, repeat_label;
...
- (id)initWithStyle:(UITableViewCellStyle)style
  ...
  if (self) {
    ...
    open_label.text = @"打开";
    open_label.backgroundColor = [UIColor clearColor];
    close_label = [[UILabel alloc] initWithFrame:CGRectMake(15, 41, 39, 21)];
    close_label.text = @"关闭";
    close_label.backgroundColor = [UIColor clearColor];
    ...
    close_field =
        [[UITextField alloc] initWithFrame:CGRectMake(56, 39, 97, 30)];
    close_field.enabled = NO;
    open_status_label =
        [[UILabel alloc] initWithFrame:CGRectMake(124, 9, 64, 21)];
    open_status_label.text = @"未运行";
    open_status_label.backgroundColor = [UIColor clearColor];
    close_status_label =
        [[UILabel alloc] initWithFrame:CGRectMake(124, 43, 64, 21)];
    close_status_label.backgroundColor = [UIColor clearColor];
...
- (void)setOpen_label:(UILabel *)a {
  if (![a isEqual:open_label]) {
    open_label = [a copy];
    UILabel *open_label1 =
        (UILabel *)[self.contentView viewWithTag:kOpen_labelValue];
...
- (void)setOpen_field:(UITextField *)a {
  if (![a isEqual:open_field]) {
    open_field = [a copy];
    UITextField *open_field1 =
        (UITextField *)[self.contentView viewWithTag:kOpen_fieldValue];
...
- (void)setOpen_status_label:(UILabel *)a {
  if (![a isEqual:open_status_label]) {
    ...
    UILabel *open_status_label1 =
        (UILabel *)[self.contentView viewWithTag:kOpen_status_labelValue];
...
- (void)setClose_label:(UILabel *)a {
  if (![a isEqual:close_label]) {
    close_label = [a copy];
    UILabel *close_label1 =
        (UILabel *)[self.contentView viewWithTag:kClose_labelValue];
...
- (void)setClose_field:(UITextField *)a {
  if (![a isEqual:close_field]) {
    close_field = [a copy];
    UITextField *close_field1 =
        (UITextField *)[self.contentView viewWithTag:kClose_fieldValue];
...
- (void)setClose_status_label:(UILabel *)a {
  if (![a isEqual:close_status_label]) {
    ...
    UILabel *close_status_label1 =
        (UILabel *)[self.contentView viewWithTag:kClose_status_labelValue];
...
- (void)handleLongPress1:(UIGestureRecognizer *)recognizer {
  if (recognizer.state == UIGestureRecognizerStateBegan) {
    [self.aDelegate timerListCell:self didHandleLongPress:recognizer];
  }
}
