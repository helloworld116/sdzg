...
@interface DevicesProfileVC ()<GCDAsyncUdpSocketDelegate>
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    dispatch_async(GLOBAL_QUEUE, ^{
        ...
        if (self.aSwitch.status == SWITCH_LOCAL ||
            ...
            NSData *timeMsg = [CC3xMessageUtil getP2SMsg19:self.aSwitch.macAddress];
            [self.udpSocket sendData:timeMsg
                              toHost:SERVER_IP
                                port:SERVER_PORT
                         withTimeout:kUDPTimeOut
        ...
        dispatch_async(GLOBAL_QUEUE, ^{
            ...
            if (self.aSwitch.status == SWITCH_LOCAL ||
                ...
                [self.udpSocket sendData:delayMsg
                      ...
                      self.aSwitch.status == SWITCH_REMOTE_LOCK){
                NSData *delayMsg = [CC3xMessageUtil getP2SMsg55:self.aSwitch.macAddress];
                [self.udpSocket sendData:delayMsg
                                  toHost:SERVER_IP
                                    port:SERVER_PORT
                             withTimeout:kUDPTimeOut
    ...
    dispatch_after(delayInNanoSeconds2, GLOBAL_QUEUE, ^{
        self.refreshTimer = [[NSTimer alloc] initWithFireDate:[NSDate date] interval:kRefreshIntveral target:self selector:@selector(checkSwitchStateInTimer) userInfo:nil repeats:YES];
...
- (void)changeOutwardBySwitchState:(BOOL)isOn{
    if (isOn) {
        ...
        self.imgViewOperation.image = [UIImage imageNamed:@"smartplug_open_button"];
    }else{
        self.imgViewBackground.image = [UIImage imageNamed:@"background1"];
        self.imgViewLight.image = [UIImage imageNamed:@"lamp_unlight"];
        self.imgViewOperation.image = [UIImage imageNamed:@"smartplug_close_button"];
    }
...
- (void)checkSwitchStateInTimer{
    ...
    if (self.aSwitch.status == SWITCH_LOCAL ||
        ...
        [self.udpSocket sendData:[CC3xMessageUtil getP2dMsg0B]
              ...
              self.aSwitch.status == SWITCH_REMOTE_LOCK){
        //开关状态
        [self.udpSocket sendData:[CC3xMessageUtil getP2SMsg0D:self.aSwitch.macAddress]
                          toHost:SERVER_IP
                            port:SERVER_PORT
                     withTimeout:kUDPTimeOut
...
- (void)sendMsg17
{
    ...
    dispatch_async(GLOBAL_QUEUE, ^{
        ...
        if (self.aSwitch.status == SWITCH_LOCAL || self.aSwitch.status == SWITCH_LOCAL_LOCK) {
            ...
            msg = [CC3xMessageUtil getP2sMsg13:self.aSwitch.macAddress aSwitch:!self.isSwitchOn];
            host = SERVER_IP;
            port = SERVER_PORT;
...
- (void)countDown {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.delayTime<=0){
            ...
            NSString *image3Name = [@"digit_" stringByAppendingFormat:@"%d",min/10];
            NSString *image4Name = [@"digit_" stringByAppendingFormat:@"%d",min%10];
...
#pragma mark 延迟操作处理
-(void)delayHandler{
    self.delayTimer = [[NSTimer alloc] initWithFireDate:[NSDate date] interval:60 target:self selector:@selector(countDown) userInfo:nil repeats:YES];
...
- (void)timeTaskHandler{
    ...
    for (CC3xTimerTask *task in self.timeTaskList) {
        ...
        if (task.timeDetail&1<<0) {
            ...
            if (task.week&(1<<weekday)) {
                //时间还未到并且操作打开
                if ((diff<task.startTime&&(task.timeDetail&1<<1))||(diff<task.endTime&&(task.timeDetail&1<<2))) {
    ...
    if (taskInTodayList) {
        ...
        NSArray *sortStartTimeList = [startTimeList sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            int num1 = (int)obj1;
            int num2 = (int)obj2;
            return num1-num2;
        }];
        NSArray *sortEndTimeList = [endTimeList sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            int num1 = (int)obj1;
            int num2 = (int)obj2;
            return num1-num2;
        }];
