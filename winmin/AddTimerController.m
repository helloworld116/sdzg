...
@interface AddTimerController ()
@property (nonatomic,strong) NSString *startTime,*endTime;//保存开始时间和结束时间，以便弹出时间选择器时选中该时间
@property (nonatomic,strong) NSDateFormatter *dateFormatter;
@end

@implementation AddTimerController
@synthesize mondayButton,tuesdayButton,wensdayButton,thursdayButton,fridayButton,saturdayButton,sundayButton,startTimeLabel,startTimeButton,startTimeSwitch,endTimeLabel,endTimeButton,endTimeSwtich,saveButton,content_View;
...
- (void)viewDidLoad
{
    ...
    //任务列表背景图片，及范围
    CGRect frame = CGRectMake(0, STATUS_HEIGHT + NAVIGATION_HEIGHT, DEVICE_WIDTH, DEVICE_HEIGHT-STATUS_HEIGHT - NAVIGATION_HEIGHT);
    content_View = [[UIView alloc]initWithFrame:frame];
    content_View.backgroundColor = [UIColor clearColor];
    [self.view addSubview:content_View];
    UIImageView * content_bg = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, DEVICE_WIDTH, DEVICE_HEIGHT-STATUS_HEIGHT - NAVIGATION_HEIGHT)];
    content_bg.image = [UIImage imageNamed:@"window_background"];
//    //    [content_bg setBounds:CGRectMake(0, 0, 260, DEVICE_HEIGHT-STATUS_HEIGHT - NAVIGATION_HEIGHT)];
    ...
    //创建socket通信
    _udpSocket =
    [[GCDAsyncUdpSocket alloc] initWithDelegate:self
    ...
    [mondayButton addTarget:self action:@selector(selectDayOfWeek:) forControlEvents:UIControlEventTouchUpInside];
    [mondayButton setTitle:@"一" forState:UIControlStateNormal];
    mondayButton.selected = NO;
    [mondayButton setBackgroundImage:[UIImage imageNamed:@"smartplug_timer_week_button_nor"] forState:UIControlStateNormal];
    ...
    [tuesdayButton addTarget:self action:@selector(selectDayOfWeek:) forControlEvents:UIControlEventTouchUpInside];
    [tuesdayButton setTitle:@"二" forState:UIControlStateNormal];
    tuesdayButton.selected = NO;
    [tuesdayButton setBackgroundImage:[UIImage imageNamed:@"smartplug_timer_week_button_nor"] forState:UIControlStateNormal];
    ...
    [wensdayButton addTarget:self action:@selector(selectDayOfWeek:) forControlEvents:UIControlEventTouchUpInside];
    [wensdayButton setTitle:@"三" forState:UIControlStateNormal];
    wensdayButton.selected = NO;
    [wensdayButton setBackgroundImage:[UIImage imageNamed:@"smartplug_timer_week_button_nor"] forState:UIControlStateNormal];
    ...
    [thursdayButton addTarget:self action:@selector(selectDayOfWeek:) forControlEvents:UIControlEventTouchUpInside];
    [thursdayButton setTitle:@"四" forState:UIControlStateNormal];
    thursdayButton.selected = NO;
    [thursdayButton setBackgroundImage:[UIImage imageNamed:@"smartplug_timer_week_button_nor"] forState:UIControlStateNormal];
    ...
    [fridayButton addTarget:self action:@selector(selectDayOfWeek:) forControlEvents:UIControlEventTouchUpInside];
    [fridayButton setTitle:@"五" forState:UIControlStateNormal];
    fridayButton.selected = NO;
    [fridayButton setBackgroundImage:[UIImage imageNamed:@"smartplug_timer_week_button_nor"] forState:UIControlStateNormal];
    ...
    [saturdayButton addTarget:self action:@selector(selectDayOfWeek:) forControlEvents:UIControlEventTouchUpInside];
    [saturdayButton setTitle:@"六" forState:UIControlStateNormal];
    saturdayButton.selected = NO;
    [saturdayButton setBackgroundImage:[UIImage imageNamed:@"smartplug_timer_week_button_nor"] forState:UIControlStateNormal];
    ...
    [sundayButton addTarget:self action:@selector(selectDayOfWeek:) forControlEvents:UIControlEventTouchUpInside];
    [sundayButton setTitle:@"日" forState:UIControlStateNormal];
    sundayButton.selected = NO;
    [sundayButton setBackgroundImage:[UIImage imageNamed:@"smartplug_timer_week_button_nor"] forState:UIControlStateNormal];
    [content_View addSubview:sundayButton];
    
    //重复label
    UILabel * repeat_label = [[UILabel alloc]initWithFrame:CGRectMake(40, 60, 80, 25)];
    repeat_label.text = @"重复";
    [content_View addSubview:repeat_label];
  
    //重复背景
    UIImageView * repeat_bg = [[UIImageView alloc]initWithFrame:CGRectMake(30, 53, 260,80)];
    ...
    [startTimeButton addTarget:self action:@selector(showTimePickerView:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [startTimeSwitch addTarget:self action:@selector(timeButtonSelected:) forControlEvents:UIControlEventTouchUpInside];
    [content_View addSubview:startTimeSwitch];
    
//    UIImageView * start_bg = [[UIImageView alloc]initWithFrame:CGRectMake(30, 150, 260, 40)];
//    start_bg.image = [UIImage imageNamed:@"smartplug_timer_repeat_background"];
//    [content_View insertSubview:start_bg atIndex:1];
    ...
    [endTimeButton addTarget:self action:@selector(showTimePickerView:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [endTimeSwtich addTarget:self action:@selector(timeButtonSelected:) forControlEvents:UIControlEventTouchUpInside];
    [content_View addSubview:endTimeSwtich];
    
//    UIImageView * end_bg = [[UIImageView alloc]initWithFrame:CGRectMake(30, 216, 260, 40)];
//    end_bg.image = [UIImage imageNamed:@"smartplug_timer_repeat_background"];
//    [content_View insertSubview:end_bg atIndex:1];
    
    //保存按钮，触发保存时间列表，发送udp
    saveButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [saveButton setFrame:CGRectMake(40, 363, 240, 30)];
    [saveButton addTarget:self action:@selector(saveTimer:) forControlEvents:UIControlEventTouchUpInside];
    [saveButton setBackgroundImage:[UIImage imageNamed:@"save_button_image"] forState:UIControlStateNormal];
    ...
    if (self.timerTask){
        ...
        self.timerTask.week & MONDAY ? [self selectDayOfWeek:self.mondayButton] : nil;
        self.timerTask.week & TUESDAY ? [self selectDayOfWeek:self.tuesdayButton] : nil;
        self.timerTask.week & WENSDAY ? [self selectDayOfWeek:self.wensdayButton] : nil;
        self.timerTask.week & THURSDAY ? [self selectDayOfWeek:self.thursdayButton] : nil;
        self.timerTask.week & FRIDAY ? [self selectDayOfWeek:self.fridayButton] : nil;
        self.timerTask.week & SATURDAY ? [self selectDayOfWeek:self.saturdayButton] : nil;
        self.timerTask.week & SUNDAY ? [self selectDayOfWeek: self.sundayButton] : nil;
        //让开始button显示当前时间
        self.startTime = [NSString stringWithFormat:@"%02d:%02d",_timerTask.startTime / 3600, (_timerTask.startTime % 3600) / 60];
        [startTimeButton setTitle:self.startTime
                              forState:UIControlStateNormal];
        //让结束button显示当前时间
        self.endTime = [NSString stringWithFormat:@"%02d:%02d",_timerTask.endTime / 3600, (_timerTask.endTime % 3600) / 60];
        [endTimeButton setTitle:self.endTime
                            forState:UIControlStateNormal];
        //
        ...
        [startTimeSwitch setImage:[UIImage imageNamed:@"selected"] forState:UIControlStateNormal];
        endTimeSwtich.selected = YES;
        [endTimeSwtich setImage:[UIImage imageNamed:@"selected"] forState:UIControlStateNormal];
        
        //获取时间显示
        self.startTime = [self.dateFormatter stringFromDate:[NSDate dateWithTimeIntervalSinceNow:5*60]];
        self.endTime = [self.dateFormatter stringFromDate:[NSDate dateWithTimeIntervalSinceNow:15*60]];
        if (self.startTimeSwitch.selected){
            [self.startTimeButton setTitle:self.startTime
                                  forState:UIControlStateNormal];
        }
        if (self.endTimeSwtich.selected){
            [self.endTimeButton setTitle:self.endTime
                                forState:UIControlStateNormal];
        }
    }
}
...
- (void)selectDayOfWeek:(id)sender
{
    ...
    if (button.selected){
        [button setBackgroundImage:[UIImage imageNamed:@"edit_week_select"] forState:UIControlStateNormal];
    } else{
        [button setBackgroundImage:[UIImage imageNamed:@"edit_week_unselect"] forState:UIControlStateNormal];
    }
}
//弹出时间选择视图
...
{
    //创建timePicker视图
    self.timePickerView = [[UIView alloc]initWithFrame:CGRectMake(0, 328,320, DEVICE_HEIGHT-328/2.0)];
    ...
    //toolbar上得按钮及分别触发的方法
    UIBarButtonItem * doneButton = [[UIBarButtonItem alloc]initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(timePickerDone:)];
   
    
    UIBarButtonItem * cancelButton = [[UIBarButtonItem alloc]initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(timePickerCancel:)];
    
    NSArray * tool_array = [[NSArray alloc]initWithObjects:doneButton,cancelButton, nil];
    [toolBar setItems:tool_array];
      [self.timePickerView addSubview:toolBar];

    //时间选择
    _datePicker = [[UIDatePicker alloc]initWithFrame:CGRectMake(0, 44, 320, DEVICE_HEIGHT-372)];
    ...
    CGRect frame = self.timePickerView.frame;
    frame.origin.y = self.view.frame.size.height -
    self.timePickerView.frame.size.height;
...
- (void)timePickerDone:(id)sender
{
    ...
    if (self.datePicker.tag == 1) {
        [self.startTimeButton setTitle:dateString
                              forState:UIControlStateNormal];
    } else {
        [self.endTimeButton setTitle:dateString
                            forState:UIControlStateNormal];
    }
...
- (void)saveTimer:(id)sender
{
    ...
    task.week = self.mondayButton.selected << 0   |
    self.tuesdayButton.selected << 1  |
    self.wensdayButton.selected << 2  |
    self.thursdayButton.selected << 3  |
    self.fridayButton.selected << 4   |
    self.saturdayButton.selected << 5 |
    ...
    if (self.startTimeSwitch.selected)
    {
        ...
        {
            NSArray *time = [self.startTimeButton.titleLabel.text componentsSeparatedByString:@":"];
            task.startTime = [time[0] doubleValue] * 3600 + [time[1] doubleValue] * 60;
        }
    } else
    {
        ...
        if (self.endTimeSwtich.selected)
    {
        ...
        {
            NSArray *time = [self.endTimeButton.titleLabel.text componentsSeparatedByString:@":"];
    ...
    NSLog(@"addtimer 481: \nstart time is %lu, end time is %lu", (unsigned long)task.startTime, (unsigned long)task.endTime);
   ...
   //???:数组timelist的元素不见了，，，，
    ...
    if (self.timerTask)
    {
        ...
        [self.timerList replaceObjectAtIndex:index
                                  withObject:task];
...
//    [comps release];
    ...
    if (self.aSwitch.status == SWITCH_LOCAL || self.aSwitch.status == SWITCH_LOCAL_LOCK) {
    ...
    } else if (self.aSwitch.status == SWITCH_REMOTE || self.aSwitch.status == SWITCH_REMOTE_LOCK) {
    ...
    [self.udpSocket sendData:msg toHost:host port:port withTimeout:kUDPTimeOut tag:tag];
}
...
- (void)back
{
//    if (self.delegate && [self.delegate respondsToSelector:@selector(changeTimerList:)])
//    {
