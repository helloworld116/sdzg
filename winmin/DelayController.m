...
@property (nonatomic,strong) UIButton *btnOfTextfeildSide;//textfeild旁边的按钮，用于text选中输入时，选中按钮切换到此按钮
@end

@implementation DelayController
@synthesize currentButton,content_view,field,delayTime,countDownTimer,startSwitch,doneButton;
...
- (void)viewDidLoad
{
    ...
    NSArray * label_name_array = [NSArray arrayWithObjects:@"5分钟",@"10分钟",@"30分钟",@"60分钟",@"90分钟",@"120分钟", nil];
    for (int i = 0; i<3; i++)
    {
        for (int j = 0; j<2; j++)
        {
            ...
            [swithBtn setImage:[UIImage imageNamed:@"unselected"] forState:UIControlStateNormal];
            [swithBtn setImage:[UIImage imageNamed:@"selected"] forState:UIControlStateSelected];
            swithBtn.tag = i*2+j+300;
            [swithBtn addTarget:self action:@selector(ButtonAction:) forControlEvents:UIControlEventTouchUpInside];
            [content_view addSubview:swithBtn];
            
            UILabel * label = [[UILabel alloc]initWithFrame:CGRectMake(79+j*109, 72+i*56, 62, 21)];
...
//    [label_name_array release];
    ...
    [switchBthLast setImage:[UIImage imageNamed:@"unselected"] forState:UIControlStateNormal];
    [switchBthLast setImage:[UIImage imageNamed:@"selected"] forState:UIControlStateSelected];
    //默认选中改按钮
    [switchBthLast setSelected:YES];
    self.currentButton = switchBthLast;
    self.btnOfTextfeildSide = switchBthLast;
    [switchBthLast addTarget:self action:@selector(ButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    ...
    UILabel * lastLabel = [[UILabel alloc] initWithFrame:CGRectMake(170, 239, 42, 21)];
    lastLabel.text = @"分钟";
    lastLabel.textColor = [UIColor blackColor];
    [content_view addSubview:lastLabel];
    
    UILabel * startLabel = [[UILabel alloc] initWithFrame:CGRectMake(49, 296, 42, 21)];
    startLabel.text = @"操作";
    startLabel.textColor = [UIColor whiteColor];
    [content_view addSubview:startLabel];
    startSwitch = [[UISwitch alloc]initWithFrame:CGRectMake(89, 291, 51, 31)];
    startSwitch.on = YES;
    [startSwitch addTarget:self action:@selector(onOrOff) forControlEvents:UIControlEventValueChanged];
    [content_view addSubview:startSwitch];
    
    UIButton * saveButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [saveButton setFrame:CGRectMake(40, 360, 240, 30)];
    [saveButton addTarget:self action:@selector(saveDelay) forControlEvents:UIControlEventTouchUpInside];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(numbKeyboardWillShow:) name:UIKeyboardWillShowNotification object:self.view.window];
}
...
//         NSString *strTime = [NSString stringWithFormat:@"%.2d时%.2d分",hours, minutes];
//            dispatch_async(dispatch_get_main_queue(), ^{
//                //设置界面的按钮显示 根据自己需求设置
...
- (void)numbKeyboardWillShow:(NSNotification *)notification
{
    ...
    [doneButton setImage:[UIImage imageNamed:@"btn_done_up"] forState:UIControlStateNormal];
    [doneButton setImage:[UIImage imageNamed:@"btn_done_down"] forState:UIControlStateHighlighted];
...
- (void)saveDelay{
    ...
    if (self.aSwitch.status == SWITCH_LOCAL || self.aSwitch.status == SWITCH_LOCAL_LOCK) {
        ...
        msg = [CC3xMessageUtil getP2SMsg4F:self.aSwitch.macAddress delay:delayTime on:startSwitch.on];
        host = SERVER_IP;
        port = SERVER_PORT;
