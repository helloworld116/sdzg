...
- (NSArray *)loadSwitches {
  ...
  for (GDataXMLElement *aSwitch in switchArray) {
    ...
    NSArray *tags = [aSwitch elementsForName:@"tag"];
    if (tags.count > 0) {
      GDataXMLElement *firstTag = (GDataXMLElement *)tags[0];
      tag = [firstTag.stringValue intValue];
    }
...
- (BOOL)saveXmlWithList:(NSArray *)switchArray {
  if (switchArray.count) {
    ...
    for (int i = 1; i <= switchArray.count; i++) {
      CC3xSwitch *aSwitch = [switchArray objectAtIndex:(i - 1)];
      ...
      GDataXMLElement *lockedEelement = [GDataXMLElement
          elementWithName:@"locked"
      ...
      GDataXMLElement *tagElement = [GDataXMLElement
          elementWithName:@"tag"
              stringValue:[NSString
                              stringWithFormat:@"%d", kSwitchBeginTag + i]];
      ...
      [switchElement addChild:tagElement];
